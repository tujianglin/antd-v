...
export const genActiveStyle = (token: ColorPickerToken, borderColor: string, outlineColor: string) => ({
...
const genClearStyle = (token: ColorPickerToken, size: number, extraStyle?: CSSObject): CSSObject => {
...
const genStatusStyle = (token: ColorPickerToken): CSSObject => {
  const { componentCls, colorError, colorWarning, colorErrorHover, colorWarningHover, colorErrorOutline, colorWarningOutline } =
    token;
  return {
...
export default genStyleHooks('ColorPicker', (token) => {
  ...
  const colorPickerToken = mergeToken<ColorPickerToken>(token, {
    ...
    colorPickerPreviewSize: token.calc(colorPickerSliderHeight).mul(2).add(marginSM).equal() as number,
  });
