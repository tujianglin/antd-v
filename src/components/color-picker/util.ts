import type { ColorGenInput } from '@/vc-component/color-picker';
import { Color as RcColor } from '@/vc-component/color-picker';

import { AggregationColor } from './color';
import type { ColorValueType } from './interface';

export const generateColor = (color: ColorGenInput<AggregationColor> | Exclude<ColorValueType, null>): AggregationColor => {
 ...
 */
export const getGradientPercentColor = (colors: { percent: number; color: string }[], percent: number): string => {
  const filledColors = [
    {
      percent: 0,
