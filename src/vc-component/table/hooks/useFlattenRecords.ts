...
): ComputedRef<FlattenData<T>[]> {
  const arr = computed<FlattenData<T>[]>(() => {
    if (expandedKeys.value?.size) {
      ...
      for (let i = 0; i < data.value?.length; i += 1) {
        ...
        fillRecords(list, record, 0, childrenColumnName.value, expandedKeys.value, getRowKey.value, i);
      }
    ...
    return data.value?.map((item, index) => {
      return {
        ...
        indent: 0,
        index,
        rowKey: getRowKey.value(item, index),
      };
    });
  });
