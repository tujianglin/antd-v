...
import setValue from '@/vc-util/utils/set';
...
export function cloneByNamePathList(store: Store, namePathList: InternalNamePath[]): Store {
  ...
  namePathList.forEach((namePath) => {
    ...
    // Skip undefined values for array elements to avoid creating sparse arrays with null
    // This happens when a list item is being removed but the field entity hasn't been fully unregistered yet
    if (value === undefined && namePath.length > 0) {
      const parentPath = namePath.slice(0, -1);
      const lastKey = namePath[namePath.length - 1];
      const parentValue = getValue(store, parentPath);

      // If parent is an array and the value is truly undefined (not just missing), skip it
      if (Array.isArray(parentValue) && typeof lastKey === 'number') {
        return;
      }
    }
